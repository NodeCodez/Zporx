...
ctypes.windll.kernel32.SetConsoleTitleW("Zprox | Tool By Node! | https://discord.gg/vZ9KccgVK6")
ua = UserAgent()
...
def get_proxies(url):
    ...
    for proxy, port in findall('((?:\d{1,3}\.){3}\d{1,3}):(\d+)', content):
        text += proxy + ":" + port + "\n"
    print(Fore.GREEN + "Loaded ", len(text.splitlines()))
    return text
...
print('Done! do you wish to check them? [y/n]')
value = input()

if value == 'y':
    os.system('python "checker/CheckX.py"')
if value == 'n':
    print('Alright they may not all work!')
else: 
    print('Error please make sure you did a lower case y or n!')
    time.sleep(3)
    exit()